...
mattered. Every number here comes from a measurement rather than an estimate —
...
two clusters is the mean over every cross-pair of their members, and it
recomputes that from the point-distance matrix every time it weighs a pair — so
...
The port took the algorithm to C essentially unchanged — `averageDistance()`
still summed over `|A| × |B|` member pairs for every candidate pair, every
iteration — but precomputed the point distances into a flat `float32[N × N]`
matrix and held the member lists as flat `int` arrays.
...
So the matrix can _maintain_ cluster-to-cluster distances rather than
recomputing them from members. A merge costs O(k) updates, and — the real win —
a find-minimum pass is now `k²/2` single float reads instead of `N²/2`. That
term finally shrinks as clusters merge: ~N³/6 total, and shrinking fast.

**Stable slot ids.** A merged cluster reuses the lower of the two slot indices
rather than appending and compacting. No index arrays to copy, no array
...
**Active-index list.** A list with swap-with-last removal holds the live slots,
...
Not speed, but it constrains what any later optimization may do, so it belongs
in the sequence:
...
- **Tie-breaking** (`d002291`). On sparse data with many identical rows — BigWig
  ...
  dendrogram. Ties now break toward the smallest combined cluster size, which
  merges tied points into a balanced binary tree instead.
- **Numerical stability** (`388be72`). Distances accumulate in `double`, and the
  Lance-Williams weights compute in `double`, because N−1 chained float32
  updates drift. Merge heights clamp to non-decreasing along any root-ward path,
  since float rounding could otherwise invert a near-tie and surface as a
...
arbitrary; the caching would have resolved them differently for no stated
reason. Slot id now breaks the final tie, making the order total and the choice
canonical. The v3.0.4 compatibility snapshots — including the tie-heavy
...
Every benchmark above ran with no progress callback, which turned out to hide
the single largest cost on the path JBrowse actually uses. `onProgress`
...
exactly the numerical stability step 3 bought. That is a bad trade for a library
whose output people publish, so this headroom deliberately stays on the table.
...
The three C generations build from their actual committed sources —
`git show <ref>:src/wasm/distance.c` — with `gcc -O2` and a stub `emscripten.h`,
linked into one harness so they run against the same buffers in the same
...
is not a like-for-like comparison — which is why the "current" column beside it
goes through WebAssembly. Treat the last table as order-of-magnitude.

Generations 2 and 4 got their own equivalence check: 28 datasets spanning
continuous, integer, all-zero and sparse-duplicate input, compared
